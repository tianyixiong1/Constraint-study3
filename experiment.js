...
const ratingTrials = selectedScenarios.map(scenario => {

  const genderKey = jsPsych.randomization.sampleWithoutReplacement(
    ['female_agent','male_agent'],1
  )[0];
  ...
  return {
    ...
    preamble: `
      <div style="text-align:left;margin-bottom:15px;">
        ...
        <p>${scenarioText}</p>
        <p>${agentName} decides to <strong>${chosenAction}</strong>.</p>
      </div>
      <div style="text-align:left;margin-bottom:10px;">
        <p>Please tell how much you agree with the following statement:</p>
        <p><em>"${agentName} had to ${chosenAction}."</em></p>
      </div>
    `,
    html: `
